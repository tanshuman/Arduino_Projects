...
void loop()
{
  ...
  else if(((l==0)&&(c1==0))||((c3==0)&&(r==0)))
  {
    ...
    if(((l==1)&&(c1==0)&&(c2==0)&&(c3==0)&&(r==0)) || ((l==1)&&(c1==1)&&(c2==0)&&(c3==0)&&(r==0)))
    {
      ...
      {
        lcd.print("right");
        directions[i]='R';    //store 'R' in the array
    ...
    else if(((l==0)&&(c1==0)&&(c2==0)&&(c3==0)&&(r==1)) || ((l==0)&&(c1==0)&&(c2==0)&&(c3==1)&&(r==1)))
    {
      ...
      else if((l==1)&&(r==1)&&(eosens())) //st and left
      {
        lcd.print("Straight");
        directions[i]='S';    //store 'S' in the array
        i++;    //increment to the next position in the array
        forward();  //forward
        delay(fdelay);  //f delay
      }
    }
    ...
    else if((l==0)&&(c1==0)&&(c2==0)&&(c3==0)&&(r==0))
    {
      ...
      {
        lcd.print("4 Way");
        directions[i]='R';  //store 'R' into the array
        i++;
        right();  //turn right
        delay(tdelay);  //turn delay
      }
...
void decisions()
{
  ...
  else if(reshortn[k]=='R')  //If element is 'L'
  {
    left();   //Right
    delay(tdelay);    //tdelay
  }
  else if(reshortn[k]=='E')  //If element is 'L'
  {
    Stop();   //Right
    lcd.print("END");
    while(1);    //tdelay
...
for(i=0;shortn[i]!='E';i++)      // Read all the element in array
{
 ...
 {
  if((shortn[i-1]=='S')&&(shortn[i+1]=='R')) // SUR is replaced by xxL
  {
   shortn[i-1]='X';
   shortn[i]='X';
   shortn[i+1]='L';
  }
   else if((shortn[i-1]=='R')&&(shortn[i+1]=='R')) // RUR is replaced by xxS
  {
   shortn[i-1]='X';
   shortn[i]='X';
   shortn[i+1]='S';
  }
  else if((shortn[i-1]=='R')&&(shortn[i+1]=='S')) // RUS is replaced by xxR
  {
   shortn[i-1]='X';
   shortn[i]='X';
   shortn[i+1]='L';
  }
   else if((shortn[i-1]=='L')&&(shortn[i+1]=='R')) // LUL is replaced by xxU
  {
   shortn[i-1]='X';
   shortn[i]='X';
   shortn[i+1]='U';
  }
    else if((shortn[i-1]=='R')&&(shortn[i+1]=='L')) // LUL is replaced by xxU
  {
   shortn[i-1]='X';
   shortn[i]='X';
   shortn[i+1]='U';
  }
    else if((shortn[i-1]=='S')&&(shortn[i+1]=='S')) // LUL is replaced by xxU
  {
   shortn[i-1]='X';
   shortn[i]='X';
   shortn[i+1]='U';
