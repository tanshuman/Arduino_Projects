...
void final()
{
  ...
  {
    lcd.print("END OF MAZE - Dead End");
    Stop();  //stop the robot
    while(1);
  }
...
void decisions()
{
  ...
  else if(reshortn[k]=='L')  //If element is 'L'
  {
    lcd.print("Right (L)");
    delay(1000);
    right();   //Left is to be replaced with right reverse
    delay(tdelay);    //tdelay
  }
  else if(reshortn[k]=='R')  //If element is 'R'
  {
    lcd.print("Left (R)");
    delay(1000);
    left();   //Right is replaced with Left for reverse
    delay(tdelay);    //tdelay
  }
  else if(reshortn[k]=='E')
  {
    lcd.print("Left (R)");
    Stop();   //END of Maze
    while(1);
...
for(i=0;shortn[i]!='E';i++)      // Read all the element in array
{
 if(shortn[i]=='U')              // Check if the element is 'U'
 {
  ...
  else if((shortn[i-1]=='R')&&(shortn[i+1]=='S')) // RUS is replaced by xxL
  {
   shortn[i-1]='X';
   shortn[i]='X';
   shortn[i+1]='L';
...
for(i=0; shortn[i]!='E';i++)  // Read every element in the array
 ...
 lcd.print("FINAL2=");
  for(j=0; reshortn[j]!='E'; j++)
   reshortn[j]=shortn[j];
 reshortn[j]='E';
 printing(reshortn);          //Print the shortest path reverse
 delay(1000);
